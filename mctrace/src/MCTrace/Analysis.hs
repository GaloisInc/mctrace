...
import MCTrace.RuntimeAPI ( probeSupportFunctions )

-- | The type of events that can be generated by the binary rewriter that are application-specific
data LogEvent = LogEvent
  deriving (Show)

instance PP.Pretty LogEvent where
  pretty le =
    case le of
      LogEvent -> PP.pretty "LogEvent"
...
data InjectedAssets globals arch =
  InjectedAssets { injectedStorePointer :: R.ConcreteAddress arch
                 ...
                 , probeSupportFunctionsPtr :: R.ConcreteAddress arch
                 -- ^ The address of the global variable holding the (array of) 
